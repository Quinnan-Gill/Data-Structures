...
public final class LinkedBag<E> implements BagInterface<E> {
    ...
    public boolean equals(Object other) {
        ...
        if (other == null) {
          return false;
        }else if (!(other instanceof ArrayBag || other instanceof LinkedBag)) {
          return false;
        }
        // other = (LinkedBag) other;
        if (((LinkedBag) other).getCurrentSize() != getCurrentSize()) {
          return false;
        }
        E[] arr = toArray();
        for(int i = 0; i < arr.length; i++) {
          if(getFrequencyOf(arr[i]) != ((LinkedBag) other).getFrequencyOf(arr[i])){
            return false;
          }
        }
        return true;
