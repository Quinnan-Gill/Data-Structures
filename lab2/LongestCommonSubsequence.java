...
public class LongestCommonSubsequence {
    ...
    public static void main(String args[]) {
        ...
        while(!possibleSubsequences.isEmpty()) {
          String testStr = possibleSubsequences.remove();
          if(longest.length() < testStr.length()) {
            if(isSubsequence(testStr, second)) {
              longest = testStr;
            } else if (testStr.length() - longest.length() >= 2){
              for(int i = testStr.length() - 1; i >= 0; i--){
                String tempStr = testStr.substring(0, i) + testStr.substring(i+1) + "";
                possibleSubsequences.add(tempStr);
              }
            }
          }
        }
